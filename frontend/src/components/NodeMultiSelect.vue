...
<template>
  <div class="flex flex-col gap-2">
    <div class="flex items-center gap-2 flex-wrap">
      <input v-model="filters.search" class="input input-bordered input-sm min-w-0 flex-1" :placeholder="i18n.t('搜索节点...')" />
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
      <select v-model="filters.source" class="select select-bordered select-sm min-w-0">
        <option value="">{{ i18n.t('全部来源') }}</option>
        <option v-for="source in sourceOptions" :key="source" :value="source">
          {{ i18n.t(nodeSourceLabel(source)) }}
        </option>
      </select>
      <select v-model="filters.country" class="select select-bordered select-sm min-w-0">
        <option value="">{{ i18n.t('全部国家') }}</option>
        <option v-for="country in countryOptions" :key="country" :value="country">{{ country }}</option>
      </select>
      <select v-model="filters.type" class="select select-bordered select-sm min-w-0">
