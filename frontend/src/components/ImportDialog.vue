...
<template>
  <div class="modal modal-open">
    <div class="modal-box max-w-2xl">
      <h3 class="font-bold text-lg mb-3">导入节点</h3>
      <div role="tablist" class="tabs tabs-boxed mb-4">
        <a role="tab" class="tab" :class="{ 'tab-active': tab === 'links' }" @click="tab = 'links'">分享链接</a>
        <a role="tab" class="tab" :class="{ 'tab-active': tab === 'subscription' }" @click="tab = 'subscription'">订阅 URL</a>
        <a role="tab" class="tab" :class="{ 'tab-active': tab === 'config' }" @click="tab = 'config'">Config JSON</a>
      </div>

      <div v-show="tab === 'links'">
        <p class="text-sm opacity-70 mb-2">每行一个 ss/vmess/vless/trojan/hysteria2/tuic 链接，或粘贴 base64 订阅内容。</p>
      ...
      <div v-show="tab === 'subscription'" class="flex flex-col gap-3">
        <label class="form-control">
          <span class="label-text mb-1">名称（可选）</span>
          <input v-model="subName" class="input input-bordered" placeholder="我的订阅" />
        </label>
        <label class="form-control">
          <span class="label-text mb-1">订阅 URL</span>
          <input v-model="subUrl" class="input input-bordered" placeholder="https://example.com/sub" />
        </label>
        <p class="text-xs opacity-60">服务端抓取，默认拒绝私网地址（SSRF 防护）。</p>
      </div>

      <div v-show="tab === 'config'">
        <p class="text-sm opacity-70 mb-2">粘贴完整 config.json，将从 outbounds 中提取代理节点（跳过 selector/direct 等分组）。</p>
